...
@lru_cache()
def b2_api():
    logger.info("Creating and authorizing B2 API")
...
def b2_upload_file(local_file: str, file_name: str):
    api = b2_api()
    logger.debug("Uploading {local_file} to B2 as {file_name}")
    ...
    logger.debug(f"Uploaded {local_file} to B2. Download URL: {download_url}")

...
from src.tests.helpers import create_post, create_comment, like_post
from src import security


@pytest.fixture()
def mock_generate_cute_creature_api(mocker):
    return mocker.patch("src.tasks._generate_cute_creature_api", return_value={"output_url": "https://fakeurl.com/image.jpg"})
...
async def test_create_post_with_prompt(async_client: AsyncClient, logged_in_token: str, mock_generate_cute_creature_api):
    ...
    mock_generate_cute_creature_api.assert_called_once()
...
async def test_like_post(async_client: AsyncClient, created_post: dict, logged_in_token: str):
    response = await async_client.post(
        "/like", json={"post_id": created_post["id"]}, headers={"Authorization": f"Bearer {logged_in_token}"}
    )
    assert response.status_code == 201
...
@pytest.mark.anyio
async def test_create_comment(async_client: AsyncClient, confirmed_user: dict, created_post: dict, logged_in_token: str):
    body = "Test Comment"

    response = await async_client.post(
        "/comment", json={"body": body, "post_id": created_post["id"]}, headers={"Authorization": f"Bearer {logged_in_token}"}
    )
    assert response.status_code == 201
    assert {
        "id": 1,
